...
group, everyone answered yes to only 1 question, a. In the fifth group, everyone
(all 1 person) answered "yes" to 1 question, b. In this example, the sum of
these counts is 3 + 0 + 1 + 1 + 1 = 6.
        ...
        ( hGetContents
            >=> ( StringC.lines
                    >>> List.groupBy
                      ( curry
                          ( uncurry (,)
                              >>> (StringC.null >>> not) *** (StringC.null >>> not)
                              >>> uncurry (&&)
                          )
                      )
                    >>> List.filter (/= [""])
...
part2 :: [] ([] StringC.ByteString) -> Int
part2 =
  fmap
    ( fmap (StringC.unpack >>> fmap Set.singleton >>> Set.unions)
        >>> inters
    )
    >>> fmap Set.size
    >>> sum
  where
    inters :: [] (Set.HashSet Char) -> Set.HashSet Char
    inters xs0 =
      if
          | [] <- xs0 -> Set.empty
          | [x] <- xs0 -> x
          | x : xs <- xs0 -> Set.intersection x (inters xs)
